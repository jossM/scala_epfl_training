...
trait FunSets extends FunSetsInterface {
   ...
   */
  def singletonSet(elem: Int): FunSet = otherElem => otherElem == elem
   ...
   */
  def diff(s: FunSet, t: FunSet): FunSet = elem => if (t(elem)) false else s(elem)
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = elem => exists(s, sElem => f(sElem) == elem)
